...
// 2. PRODUCTS (PUBLIC WITH FILTERS)
app.get('/products', (req, res) => {
  const cat = req.query.category || "All";
  const filtered = cat === "All" ? products : products.filter(p => p.category === cat);
  
  const categories = ["All", "Electronics", "Groceries", "Fashion", "Home"];
  ...
  const cards = filtered.map(p => `
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div class="p-5">
        <span class="text-xs font-bold text-indigo-400 uppercase">${p.category || 'General'}</span>
        <h3 class="text-lg font-bold text-gray-800">${p.name}</h3>
        <p class="text-indigo-600 font-bold text-xl mt-1">$${p.price}</p>
        <a href="https://wa.me/2349127603945" class="block mt-4 text-center bg-green-500 text-white py-2 rounded-lg font-semibold">Order via WhatsApp</a>
      </div>
    </div>
  `).join('');

  res.send('<html><head><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-50 p-6"><div class="max-w-6xl mx-auto"><h2 class="text-3xl font-black mb-6 text-gray-900">Marketplace</h2><div class="flex flex-wrap gap-2 mb-10">' + filterButtons + '</div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">' + (cards || '<p class="text-gray-400">No products found in this category.</p>') + '</div></div></body></html>');
});

// 3. ADMIN PANEL (WITH CATEGORY SELECT)
app.get('/admin', isAdmin, (req, res) => {
  ...
  const manageList = products.map(p => `
    <div class="flex justify-between items-center bg-gray-50 p-3 rounded-lg border mb-2">
      <span class="font-medium text-sm">${p.name} (${p.category})</span>
  ...
  res.send('<html><head><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-100 p-6"><div class="max-w-2xl mx-auto space-y-8"><div class="bg-white p-8 rounded-xl shadow-lg"><h2 class="text-2xl font-bold mb-6 text-gray-800">Add Product</h2><form action="/add-product" method="POST" class="space-y-4"><input type="hidden" name="pwd" value="' + pwd + '"><input type="text" name="itemName" placeholder="Product Name" class="w-full border p-2 rounded-lg" required><input type="number" name="itemPrice" placeholder="Price" class="w-full border p-2 rounded-lg" required><select name="itemCategory" class="w-full border p-2 rounded-lg"><option>Electronics</option><option>Groceries</option><option>Fashion</option><option>Home</option></select><input type="text" name="itemImage" placeholder="Image URL" class="w-full border p-2 rounded-lg"><button class="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold">Publish</button></form></div><div class="bg-white p-8 rounded-xl shadow-lg"><h2 class="text-2xl font-bold mb-6 text-gray-800">Manage Inventory</h2>' + manageList + '</div></div></body></html>');
});
